...
// OutputMode represents how the function should behave with regards to overwritting data.
type OutputMode int
...
// src/ directory. These locations can be resticted through the given pathFilter.
...
func main() {
	// Until a CLI is added, modify these three variables for local assisted code editing.
	...
	actions := []Action{
		RemoveContactsAction([]string{
			"notAnEmail@google.com",
		}),
	}
	filters := []Filter{
		TestFilter([]string{
			"tast.packageNameHere.testNameHere",
		}),
	}
	...
	err := WalkAllKnownTests(defaultSrcDir, pathFilter, filters, actions, ModeDryRunVerbose)
	if err != nil {
		fmt.Println(err)
		return
	}
