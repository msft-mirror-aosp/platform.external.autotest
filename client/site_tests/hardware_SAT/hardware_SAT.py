...
def memory_channel_args_snb_bdw(channel_modules):
    ...
    if (mad_chnl >> 4) & 3 != 2:
        raise error.TestError(
                'This test does not support triple-channel mode.')
    ...
    if not channel_hash & (1 << 23):
        logging.warning(
                'Memory channel_hash deactivated... going with cache-line '
    ...
    return (' --memory_channel %s --memory_channel %s --channel_hash 0x%x'
            ' --channel_width 64' %
            (','.join(channel_modules[channel_order[0]]), ','.join(
                    channel_modules[channel_order[1]]), channel_hash))
...
class hardware_SAT(test.test):
    ...
    def run_once(self,
        ...
        # and should catch clearly fautly memory. 4 hours
