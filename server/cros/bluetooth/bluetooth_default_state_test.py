...
class bluetooth_Health_DefaultStateTest(
    ...
    """This class implements the default state test and helper functions."""
    ...
    def default_state_test(self):
        ...
        (address, bluetooth_version, manufacturer_id, supported_settings,
         current_settings, class_of_device, name, short_name) = self.read_info()
        ...
        if not self.floss:
            ...
            if bluez_properties['Powered']:
                raise error.TestFail(
                        'Bluetooth daemon Powered property does not '
                        'match kernel while powered off')
            if not self.compare_property(
                    bluez_properties['Discoverable'],
            ...
            if self.bluetooth_facade.is_bluetoothd_proxy_valid():
                raise error.TestFail('Bluetooth daemon is still powered')
        ...
        if not self.floss:
            ...
            if not self.compare_property(
                    bluez_properties['Pairable'],
                    bluetooth_socket.MGMT_SETTING_PAIRABLE, current_settings):
                raise error.TestFail(
                        'Bluetooth daemon Pairable property does not '
                        'match kernel while powered on')
            if not self.compare_property(
                    bluez_properties['Discoverable'],
                    bluetooth_socket.MGMT_SETTING_DISCOVERABLE,
            ...
            if not self.bluetooth_facade.is_bluetoothd_proxy_valid():
                raise error.TestFail('Bluetooth daemon is not powered')

            if self.bluetooth_facade.is_discovering():
                raise error.TestFail('Bluetooth daemon believes adapter is '
                                     'discovering while powered on')
        ...
        if not flags & bluetooth_socket.HCI_RUNNING:
            raise error.TestFail('HCI RUNNING flag does not match kernel while '
                                 'powered on')
        if bool(flags & bluetooth_socket.HCI_ISCAN) != bool(
                bluez_properties['Discoverable']):
        ...
        # If the kernel does not support the BR/EDR allowlist, the adapter
        # should generically connectable, so it should be in PSCAN
        ...
        # flag come and go.
        if supports_add_device and not self.floss:
            ...
            time.sleep(3)
            current_settings = self.read_info()[4]
            self.log_settings("After add device", current_settings)
            ...
            # on older devices. Wait for few second before reading the
            # settings.
            time.sleep(3)
            current_settings = self.read_info()[4]
        ...
        elif supports_add_device and self.floss:
            # If PSCAN is currently on, then device is CONNECTABLE
            # or a previous add device which was not removed.
            # Turn on and off DISCOVERABLE to turn off CONNECTABLE and
            # PSCAN
            if flags & bluetooth_socket.HCI_PSCAN:
                ...
                if flags & bluetooth_socket.HCI_PSCAN:
                    raise error.TestFail(
                        'PSCAN on after toggling DISCOVERABLE')
