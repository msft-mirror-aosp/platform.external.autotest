...
from autotest_lib.client.common_lib.cros.autotestChrome.py_utils import atexit_with_log

_TSPROXY_PATH = os.path.join(
    py_utils.GetCatapultDir(), 'cros', 'autotestChrome', 'tsproxy.py')
...
class TsProxyServer(object):
  ...
  def StartServer(self, timeout=30, retries=None):
