...
class Servo(object):
    ...
    def set_usb3_power(self, state):
        """Set power of USB3 ports on servo to state, either 'on' or 'off'.
        ...
        """
        if state != 'off' and state != 'on':
            raise error.TestError('Unknown USB3 power state request: %s' %
                                  state)

        if not self.supports_usb3_power_control():
            logging.info(
                    'Not a supported servo setup. Unable to set USB3 power'
                    'state %s.', state)
            return

        self.set_nocheck('usb3_pwr_en', state)

    def supports_usb3_mux_control(self):
        """True if servo supports mux control of USB3 ports on servo."""
        return self.has_control('usb3_mux_en')

    def set_usb3_mux(self, state):
        """Set USB3 mux on servo to state, eitehr 'on' or 'off'.
        ...
        """
        if state != 'off' and state != 'on':
            raise error.TestError('Unknown USB3 mux state request: %s' % state)

        if not self.supports_usb3_mux_control():
            logging.info(
                    'Not a supported servo setup. Unable to set USB3 mux'
                    'state %s.', state)
            return

        self.set_nocheck('usb3_mux_en', state)
