...
_XTS_NAME = 'cts'
_BRANCH_NAME = 'aosp-android13-tests-dev'

# TODO(b/256108932): Add --test_config flag for test upload to gs.
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Update the preview version number in bundle_url_config and upload to gs if necessary.',
        formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument(
            '--to',
            dest='preview_version',
            default=None,
            help='If updating with the latest preview version of CTS, this option is not required.'
    ...
    if args.preview_version or args.to_latest:
        uprev_preview_version_common.main(config_path, _XTS_NAME, _BRANCH_NAME, args.preview_version)
    else:
        parser.print_help()
