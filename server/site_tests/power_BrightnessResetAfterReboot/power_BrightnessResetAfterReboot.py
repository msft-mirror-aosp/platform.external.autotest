    ...
    def run_once(self, host, client_autotest):
        ...
        if num_als_pref and not als_exists:
            ...
            if model in IGNORE_ALS_PREF_MODELS:
                logging.info('%s However, skip this check for model: %s.', msg,
                             model)
            else:
        ...
        if (initial_brightness < brightness_range[0]
                    or initial_brightness > brightness_range[1]):
        ...
        if als_exists:
            ...
            if (brightness_range_after_reboot[0] <= brightness_after_reboot <=
